...
export function createNativeSvgRenderer(host: HTMLElement, callbacks: RendererCallbacks): CandidateRenderer {
  ...
  const updateNode = (node: NativeNode, primitive: ScenePrimitive, current: MechanismScene): void => {
    ...
    if (primitive.type === 'segment') {
      ...
      const visible = group.children[0] as SVGCircleElement; const hit = group.children[1] as SVGCircleElement; const point = primitive.type === 'circle' ? primitive.center : primitive.at; const p = project(point, current.viewport); const radius = primitive.type === 'circle' ? primitive.radius / (current.viewport.maxX-current.viewport.minX) * SVG_WIDTH : 7;
      visible.setAttribute('cx', String(p.x)); visible.setAttribute('cy', String(p.y)); visible.setAttribute('r', String(radius)); visible.setAttribute('class', sceneClassName(primitive.classes, selected)); visible.setAttribute('stroke-width', String(primitive.width ?? 2));

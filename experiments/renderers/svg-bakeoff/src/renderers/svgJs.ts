...
export function createSvgJsRenderer(host: HTMLElement, callbacks: RendererCallbacks): CandidateRenderer {
  ...
  const update=(node:SvgJsNode,primitive:ScenePrimitive,current:MechanismScene)=>{
    ...
    else if(primitive.type==='circle'||primitive.type==='handle'){const point=primitive.type==='circle'?primitive.center:primitive.at;const p=project(point,current.viewport);const radius=primitive.type==='circle'?primitive.radius/(current.viewport.maxX-current.viewport.minX)*SVG_WIDTH:7;const strokeWidth=primitive.type==='circle'?(primitive.width??2):2;node.visible.center(p.x,p.y).radius(radius).attr({class:sceneClassName(primitive.classes,selected),'stroke-width':strokeWidth});node.hit.center(p.x,p.y).radius(Math.max(radius+9,16));if(primitive.type==='handle'){handlePoints.set(primitive.id,primitive.at);node.visible.attr({'data-handle':primitive.handle,class:`${sceneClassName(primitive.classes,selected)} interaction-handle`});}}
